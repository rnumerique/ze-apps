        ...
        <div class="pull-right form-inline" ng-class="!isEmpty() ? 'bg-warning' : ''">
            ...
            <span ng-repeat="item in filters.main">
                <span ng-if="item.format == 'input'">
                    <input type="{{item.type}}" class="form-control input-sm" ng-model="model[item.field]" placeholder="{{item.label}}" ng-change="update_view()()" ng-model-options="{debounce: 250}">
                </span>
                <span ng-if="item.format == 'checkbox'">
                    <label class="small">
                        {{item.label}}
                        <input type="checkbox" class="form-control input-sm" ng-model="model[item.field]" ng-change="update_view()()">
                    </label>
                </span>
                <span ng-if="item.format == 'select'">
                    <label class="small">{{item.label}}</label>
                    <select ng-model="model[item.field]" class="form-control input-sm" ng-change="update_view()()">
                        <option value="">-</option>
            ...
            <div class="row">
                <div class="col-md-{{item.size}}" ng-repeat="item in filters.secondaries">
                    <div class="form-group" ng-if="item.format == 'input'">
                        ...
                        <input type="{{item.type}}" class="form-control" ng-model="model[item.field]" ng-change="update_view()()" ng-model-options="{debounce: 250}">
                    </div>
                    <span ng-if="item.format == 'checkbox'">
                        <label class="small">
                            <input type="checkbox" class="form-control input-sm" ng-model="model[item.field]" ng-change="update_view()()">
                    ...
                    <div class="form-group" ng-if="item.format == 'select'">
                        <label>{{item.label}}</label>
                        <select ng-model="model[item.field]" class="form-control" ng-change="update_view()()">
                            <option value="">-</option>
                            <option ng-repeat="option in item.options" value="{{option.id}}">
                                {{ option.label }}
                            </option>
